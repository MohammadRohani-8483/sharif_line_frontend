...
function Page({ params }: { params: { Id: string } })  {
    ...
    const questionnaireURL = `/question/group-questionnaire/${params.Id}`
    let LinkURL = `/answer/answerlink/${params.Id}`


    let [activeOrNot,setQuestionnaireActive]=useState(true)
    
const chengePage=(page:number)=>{
    LinkURL=`/answer/answerlink/${params.Id}`+'/?page='+page
    QSLinkQuery.refetch()
}
...
<StatusQuestionnaire active={activeOrNot}>
    ...
    <CommunityStatus>
        ...
        <DisposableLink group_id={params.Id} data={DisposableLinkData?.results} status={true}></DisposableLink>
     <div className="space"></div>
    
        <Paginate activePage={1} pageSize={12} itemsCount={DisposableLinkData?.count || 0} setPage={(page)=>chengePage(page)}/>
    
    </CommunityStatus>
    <Version data={questionnaireData?.versions} setActiveVersion={(number:number)=>setActiveVersion(number)} activeVersion={activeVersion}/>
      <div className='btnMain'>

      <button onClick={()=>PatchHandler()} className='BTNsave'><p>
        ذخیره
        </p>
        </button>
      </div>

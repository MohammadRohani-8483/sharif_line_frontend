...
import { T_QuestionResponse, T_TitleAndDesc } from '@/src/utils/types/pages/answer'
...
const Page = async ({ params }: { params: { questionnaireID: string } }) => {
  ...
  try {
    const response = await axiosInstance(tokens).get<T_TitleAndDesc>(`question/title/${params.questionnaireID}`)
    titleAndDescQuestionnaire = response.data
  } catch (error: any) {
    console.log(error)
    notFound()
  }
  return <AnswerPage res={questionJson} titleAndDesc={titleAndDescQuestionnaire} error={textError} />
}

...
export const QuestionnaireSettingPageHeader = (p: Props) => {
  ...
  const deleteQuestionnaireQuery = useMutation({
    mutationFn: async () =>
      await axiosInstance().delete(`question/group-questionnaire/${p.groupID}/questionnaire/${p.questionnaireID}`)
  })
  ...
  return <PageHeader>
    ...
    <PageHeaderButtons>
      ...
      <Link href={`/${p.questionnaireID}/answer`}>
        <Tooltip title={'مشاهده'} placement={'bottom'} anchorClassName={'activate-button-anchor'}>
          <PageHeaderButton className={'activate-button-anchor'}>
            <Icon name={p.isActive ? 'eye' : 'eye_closed'} width={24} height={24} />
          </PageHeaderButton>
        </Tooltip>
      </Link>
      {pathname.includes('/setting') ?
        ...
        :
        <Link href={`/${p.groupID}/setting/${p.questionnaireID}`}>
          <Tooltip title={'مشاهده'} placement={'bottom'} anchorClassName={'activate-button-anchor'}>
            <PageHeaderButton className={'activate-button-anchor'}>
              <Icon name={p.isActive ? 'eye' : 'eye_closed'} width={24} height={24} />
            </PageHeaderButton>

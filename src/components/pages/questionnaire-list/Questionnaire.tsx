...
import {
  ...
  FootItem,
  HeadItem,
} from "@/src/styles/pages/questionnaire";
...
const Questionnaire = ({
  ...
  const restoreQuestionnaire = () => {
    axiosInstance()
      .post<{ message: string }>("question/archive/update_deleted_at/", {
      ...
      >
        <Body className={"questionnaireBody"} mode={mode}>
          <HeadItem mode={mode} isArchive={isArchive}>
            ...
            <h1 className="questionnaire_title">{data.title}</h1>
          </HeadItem>
          <Footer mode={mode}>
            {!isArchive && (
              <FootItem mode={mode}>
                ...
                <p>{formatNumber(data.results_count)}</p>
              </FootItem>
            )}
            <FootItem mode={mode}>
              <div>
            ...
            <FootItem mode={mode}>
              ...
              <p>{convertDate(data.group_created_at)}</p>
            </FootItem>
          </Footer>
        </Body>
        <Buttons mode={mode} className={"questionnaireFooterButtons"}>
          {isArchive ? (
            <Tooltip
              title="بازگرداندن"
              anchorClassName={"restore-" + data.id}
              placement={"bottom"}
            >
              <section
                onClick={restoreQuestionnaire}
                className={"restore-" + data.id}
            ...
            <>
              <Tooltip
                title={"نمودار"}
                anchorClassName={"chart-" + data.id}
                placement={"bottom"}
              >
                <Link
                  href={`/${data.id}/statistics?group_id=${data.group_id}`}
                  className={"chart-" + data.id}
              ...
              <Tooltip
                title="مشاهده"
                anchorClassName={"eye-" + data.id}
                placement={"bottom"}
              >
                <Link
                  href={`/${data.group_slug}/answer`}
                  className={"eye-" + data.id}
                  onClick={setSearchNull}
                >
              ...
              <Tooltip
                title="تنظیمات"
                anchorClassName={"setting-" + data.id}
                placement={"bottom"}
              >
                <Link
                  href={`/${data.group_id}/setting/`}
                  className={"settingButton setting-" + data.id}
                  onClick={setSearchNull}
                >
              ...
              <section
                ...
                onClick={() => setOpenPopup({ open: true, visible: true })}
              >

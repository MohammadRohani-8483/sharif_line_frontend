...
const TemplatePopup: FC<TemplatePopupProps> = memo(({
    onClose,
    setFlag
}) => {
    ...
    useEffect(() => {
        axiosInstance().get('question/template/')
            .then(res => {
    ...
    const generateTemplate = (close: () => void) => {
        axiosInstance().post<{ questionnaire_id: string }>('question/template/', { id: active })
            .then(() => {
        ...
        <Popup BoxStyle={{ width: 1080, zIndex: 20 }} dismisable onClose={onClose}>
            {(close) => (
                <Container>
                    ...
                    {!templates ?
                        <Loading>{error || <SpinnerLoading color='#2979FF' width={70} height={70} />}</Loading>
                        :
                        <Body>
                                ...
                                <Template key={template.id} $active={template.id === active} onClick={() => handleClick(template.id)}>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: "8px" }}>
                                        ...
                                        <FootItem>
                                            ...
                                            <p>
                                                {convertDate(template.created_at)}
                                            </p>
                                        </FootItem>
                                    </div>
                                </Template>
